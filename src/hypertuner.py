...
def grid_search(
    ...
    for lr, weight_decay, dropout, hidden_dim in tqdm(list(product(*grid.values())), desc=desc):
...
def rmia_offline_interp_param_search(
    ...
    target_samples, population = datasetup.target_shadow_split(dataset=dataset, split='disjoint')
